#ifndef _ROS_SERVICE_MultTwoInts_h
#define _ROS_SERVICE_MultTwoInts_h
#include <stdint.h>
#include <string.h>
#include <stdlib.h>
#include "ros/msg.h"
...
static const char MULTTWOINTS[] = "ros_essentials_cpp/MultTwoInts";

  class MultTwoIntsRequest : public ros::Msg
  {
    public:
    ...
    virtual const char * getType() override { return MULTTWOINTS; };
    virtual const char * getMD5() override { return "36d09b846be0b371c5f190354dd3153e"; };

  };

  class MultTwoIntsResponse : public ros::Msg
  {
    public:
      typedef int64_t _mult_type;
      _mult_type mult;
    ...
    virtual int serialize(unsigned char *outbuffer) const override
    {
      ...
      u_mult.real = this->mult;
      *(outbuffer + offset + 0) = (u_mult.base >> (8 * 0)) & 0xFF;
      *(outbuffer + offset + 1) = (u_mult.base >> (8 * 1)) & 0xFF;
      *(outbuffer + offset + 2) = (u_mult.base >> (8 * 2)) & 0xFF;
      *(outbuffer + offset + 3) = (u_mult.base >> (8 * 3)) & 0xFF;
      *(outbuffer + offset + 4) = (u_mult.base >> (8 * 4)) & 0xFF;
      *(outbuffer + offset + 5) = (u_mult.base >> (8 * 5)) & 0xFF;
      *(outbuffer + offset + 6) = (u_mult.base >> (8 * 6)) & 0xFF;
      *(outbuffer + offset + 7) = (u_mult.base >> (8 * 7)) & 0xFF;
      offset += sizeof(this->mult);
      return offset;
    }
    ...
    virtual int deserialize(unsigned char *inbuffer) override
    {
      ...
      u_mult.base = 0;
      u_mult.base |= ((uint64_t) (*(inbuffer + offset + 0))) << (8 * 0);
      u_mult.base |= ((uint64_t) (*(inbuffer + offset + 1))) << (8 * 1);
      u_mult.base |= ((uint64_t) (*(inbuffer + offset + 2))) << (8 * 2);
      u_mult.base |= ((uint64_t) (*(inbuffer + offset + 3))) << (8 * 3);
      u_mult.base |= ((uint64_t) (*(inbuffer + offset + 4))) << (8 * 4);
      u_mult.base |= ((uint64_t) (*(inbuffer + offset + 5))) << (8 * 5);
      u_mult.base |= ((uint64_t) (*(inbuffer + offset + 6))) << (8 * 6);
      u_mult.base |= ((uint64_t) (*(inbuffer + offset + 7))) << (8 * 7);
      this->mult = u_mult.real;
      offset += sizeof(this->mult);
     return offset;
    }

    virtual const char * getType() override { return MULTTWOINTS; };
    virtual const char * getMD5() override { return "d0f28d7162d1faf61932820980584245"; };
